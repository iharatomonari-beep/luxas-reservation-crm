-- phaseC2: オンライン予約RPCに端末種別 p_device を追加＋profile に source/onlineDevice を記録
-- 目的:
--  (1) 日次集計「オンライン(PC)/(携帯)」内訳のため、予約確定時に端末種別を保存する。
--  (2) 既存の穴の修正: 本RPCで作った予約は profile が空で source="online" がアプリ側に復元されず、
--      オンライン予約数の集計から漏れていた。今後の予約は profile.source="online" を必ず書く。
-- 適用: オーナーが Supabase SQL Editor で実行（冪等）。
--       ★未適用でもアプリは旧8引数で自動フォールバックするためクラッシュしない（端末内訳が記録されないだけ）。
-- 注意: 同名8引数版を残すと名前付き引数の解決が曖昧になるため、旧シグネチャは drop して 9引数版に作り直す。
--       p_device は default null なので旧引数呼び出しもそのまま動く。

drop function if exists public.create_online_booking_v2(text, uuid, date, time, text, text, text, text);
...
begin
  ...
  -- profile: アプリ(reservation-mapper)が復元する拡張項目。source は必ず、端末種別は妥当値のときだけ書く。
  v_profile := jsonb_build_object('source', 'online');
  if p_device in ('pc', 'mobile') then
    v_profile := v_profile || jsonb_build_object('onlineDevice', p_device);
  end if;

  -- 予約作成（room_id null = 容量ベース）。
  insert into public.reservations
    (tenant_id, store_id, customer_id, customer_name, customer_phone, staff_id, service_id, room_id,
     reservation_date, start_time, end_time, status, memo, profile)
  values
    (v_tenant, v_store, v_customer, coalesce(nullif(trim(p_customer_name), ''), 'ゲスト'), p_customer_phone,
     v_staff, p_service_id, null, p_date, p_start, v_end, 'booked', 'online', v_profile)
...
revoke all on function public.create_online_booking_v2(text, uuid, date, time, text, text, text, text, text) from public;
grant execute on function public.create_online_booking_v2(text, uuid, date, time, text, text, text, text, text) to anon, authenticated;
