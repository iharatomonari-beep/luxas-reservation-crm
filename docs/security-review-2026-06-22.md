...
## 10. Codex 第4回クロスチェックと追加対応

第4回で「店舗スコープは一貫」とまだ言えないと指摘。actionable な5点を修正、本丸2点は据え置き（理由を明記）。

### 修正した（`8b89c51` / `ca651a0`）
- **マイページのIDOR緩和（Critical由来）**: `myReservations` が `customerId` のみで他店予約も表示/キャンセル対象だった。`useMyPageData(storeId)` にして `isReservationInStore` で現在店舗の予約に限定。
- **出勤表が全店シフト表示（Medium）**: daily-ops の `dayShifts` を `filterShiftsByStore` で限定。
- **メニューCSV import が全店共通になる（High）**: 取り込みメニューに `storeScope:"selected"/storeIds:[currentStoreId]` を既定付与。
- **ヘルプ勤務の編集退行（Medium）**: シフト2画面の表示対象を「所属スタッフ＋現在店舗に応援シフトを持つスタッフ」に拡張。
- **予約CSV import が全店マスタ参照（High）**: import 用に現在店舗スコープのスタッフ/メニュー/予約と名前・IDマップを用意し、preview/parse/解決に使用（ブースは店舗スコープ未対応で全体のまま）。

### 据え置き（本丸＝モデル/サーバー判断が必要）
- **顧客PII/カルテがグローバル（Critical）**: `homeStoreId` で機械的に絞れるが、既存顧客は未設定で全員既定店舗扱い→**他店で顧客一覧が空**になる重大な挙動変更。かつ「顧客は店舗専属か全社共有か」という製品・データモデルの判断を伴う。**勝手にスコープ化せず、顧客-店舗関連＋RLSで設計する**のが正道（要ユーザー判断）。
- **会員セッション偽装（Critical・IDOR）**: `localStorage["luxas-book-member-id"]` の偽装はサーバー認証が無い限り根本対処不可。マイページの店舗絞りで露出は緩和したが、本対処は Supabase Auth ＋ サーバー側所有権検証。

### 収束した結論
4回のクロスチェックで、**クライアント側で塞げる店舗スコープのもぐら叩きはほぼ完了**（残るは顧客モデルとブース）。**残るCritical（顧客PIIのテナント分離・会員/管理者の認証認可・IDOR・RLS）はいずれもサーバー/データモデルの本丸**であり、外販前にデータ層を Supabase へ移し RLS＋tenant_id/顧客-店舗関連で強制するのが唯一の根本解決。

第4回関連コミット: `8b89c51`（マイページ/出勤/メニュー import/ヘルプ勤務）/ `ca651a0`（予約 import 解決）。
